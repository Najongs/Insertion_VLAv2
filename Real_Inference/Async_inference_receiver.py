...
Usage:
    ...
    python Async_inference_receiver.py --checkpoint checkpoints/regression_best.pt --auto-start --vl-reuse 4 --task-name yellow_point
...
class AsyncVLAInferenceEngine:
    ...
    def predict_action(self, sensor_data: torch.Tensor = None, robot_states: torch.Tensor = None,
        ...
        # Model inference with detailed timing
        with torch.autocast(self.device.type, dtype=torch.bfloat16):
            if self.config.MODEL_TYPE == 'flow_matching':
                ...
                if sensor_batch is not None:
                    sensor_features = self.model.sensor_encoder(sensor_batch)
                ...
                if robot_batch is not None:
                    robot_state_features = self.model.robot_state_encoder(robot_batch)
                ...
                pred_actions, delta = self.model.action_expert(
                    vl_features, z_chunk, sensor_features, robot_state_features
                )
                timings['action_prediction'] = (time.time() - t_action_start) * 1000
...
def main():
    ...
    parser.add_argument('--model-type', type=str, default='flow_matching',
                       choices=['flow_matching', 'regression'],
    ...
    parser.add_argument('--start-joints', type=float, nargs=6, default=[191, 1, 309, 1, 91, 1], 
                       help='Start joint positions for auto-start (default: [0,0,0,0,0,0])') #168.387659, 36.190885, 250.816119, 178.747768, 19.072951, 144.284313 # 190, 2, 308, 0, 90, 0
